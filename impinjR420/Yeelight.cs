...
namespace YeelightCMD
{
    public class Yeelight
    {
        ...
        public bool set_rgb(int red, int green, int blue)
        {
            int rgb = (red * 65536) + (green * 256) + blue;
            // 命令
            byte[] buf = Encoding.Default.GetBytes("{\"id\": " + this.id + ", \"method\": \"set_rgb\", \"params\":[" + rgb.ToString() + ",\"smooth\",500]}\r\n");
            // 发送
            this.tcpClient.Client.Send(buf);
            // 接收回应
            byte[] recv = new byte[1000];
        ...
        public bool set_power(int onoff)
        {
            string state;
            if (onoff == 1)
            {
            ...
            // 命令
            byte[] buf = Encoding.Default.GetBytes("{\"id\": " + this.id + ", \"method\": \"set_power\", \"params\":[\"" + state + "\",\"smooth\",500]}\r\n");
            // 发送
            this.tcpClient.Client.Send(buf);
            // 接收回应
            byte[] recv = new byte[1000];
